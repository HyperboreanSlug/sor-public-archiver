...
| `reports/` | `fetcher_*`, `identity_gate` + `identity_audit` (NUCLEAR: name+DOB+photo+link verification; never wrong person), `pdf_fields`, util, photos; `public_links` (FDLE + MA URL fixes). CLI: `scripts/verify_identity_full.py`. Crime summary: `crime_summary*.py`. NV multi-col crime + Start Date county repair: `scripts/repair_nv_crime_county.py`. VA vspsor card-header offenses: `fetcher_crime_va.py` + `scripts/repair_va_crime_from_html.py` |
