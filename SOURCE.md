...
| Package | Composition |
|---------|-------------|
| `database/` | schema, inserts, queries, `dedupe_*` (name+DOB uses normalize_dob for US/ISO formats), `csv_*` (incl. `csv_repair_fl_sor`), sources + `sources_merge` / `sources_race_verify` (HTML chart race consensus), deepface_scans |
