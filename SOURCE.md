...
| Package | Composition |
|---------|-------------|
| `database/` | schema, inserts, queries, `dedupe_*` (name+DOB uses normalize_dob; URL dedupe respects identity hard-reject), `csv_*` (incl. `csv_repair_fl_sor`), sources + `sources_merge` / `sources_race_verify` (HTML consensus = listed race; scrub wrong PERSON_NBR bulk race), deepface_scans |
